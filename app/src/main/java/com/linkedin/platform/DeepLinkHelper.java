...
public class DeepLinkHelper {
    ...
    public void openOtherProfile(@NonNull Context context, String memberId, DeepLinkListener callback) {
        this.deepLinkListener = callback;
        ...
        try {
            if (!LIAppVersion.isLIAppCurrent(context)) {
                return;
            }
            deepLinkToProfile(context, memberId, session.getAccessToken());
        } catch (ActivityNotFoundException e) {
            callback.onDeepLinkError(new LIDeepLinkError(LIAppErrorCode.LINKEDIN_APP_NOT_FOUND,
                    "LinkedIn app needs to be either installed or` updated"));
            deepLinkListener = null;
    ...
    private void deepLinkToProfile(@NonNull Context context, String memberId, @NonNull AccessToken accessToken) {
        Intent i = new Intent("android.intent.action.VIEW");
        Uri.Builder uriBuilder = new Uri.Builder();
        uriBuilder.scheme("linkedin");
        if (CURRENTLY_LOGGED_IN_MEMBER.equals(memberId)) {
